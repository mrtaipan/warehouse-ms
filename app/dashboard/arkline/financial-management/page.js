...
export default function ArklineFinancialManagementPage() {
  ...
  async function loadWorkspace() {
    ...
    const [{ data: rolePermissions, error: permissionError }, { data: categoryRows, error: categoryError }, { data: claimRows, error: claimError }] =
      await Promise.all([
        ...
        supabase
          .from('arkline_reimbursement_claims')
          .select(
            `
              id,
              claim_number,
    ...
    setAccess(getArklineFeatureAccess(role, permissions, isAdmin))
  ...
  const visibleClaims = useMemo(
    ...
    [access.reimbursementApprove, claims, isClaimOwner]
  )
  ...
  }, [categoryFilter, search, visibleClaims])
      ...
      return [
        {
          key: 'SUBMITTED',
          label: 'Submitted',
          items: filteredClaims.filter((item) => item.status === 'SUBMITTED'),
          badgeCount: filteredClaims.filter((item) => item.status === 'SUBMITTED').length,
        },
        {
          key: 'APPROVED',
          label: 'Approved',
  ...
  async function handleSaveBatch() {
    ...
    try {
      for (const row of batchRows) {
        ...
        const { error: attachmentInsertError } = await supabase.from('arkline_reimbursement_attachments').insert(uploadResult.attachmentRows)
      ...
      if (createdClaimIds.length) {
        await supabase.from('arkline_reimbursement_claims').delete().in('id', createdClaimIds)
      }
  ...
  async function handleSaveEditClaim() {
    ...
    try {
      const { error: updateError } = await supabase.from('arkline_reimbursement_claims').update(payload).eq('id', editingClaim.id)
  ...
  async function handleMarkPaid(claim) {
    ...
    try {
      ...
      if (uploadedPaths.length) {
        await supabase.from('arkline_reimbursement_attachments').delete().eq('claim_id', claim.id).in('storage_path', uploadedPaths)
      }
  ...
  async function handleMarkPaidGroup(group) {
    ...
    try {
      ...
      if (uploadedPaths.length) {
        await supabase.storage.from(REIMBURSEMENT_BUCKET).remove(uploadedPaths)
        await supabase.from('arkline_reimbursement_attachments').delete().in('storage_path', uploadedPaths)
      }
  ...
  async function handleDeleteClaim(claim) {
    ...
    try {
      ...
      const { error: deleteError } = await supabase.from('arkline_reimbursement_claims').delete().eq('id', claim.id)
  ...
  const canView = access.reimbursementView
  const canSubmit = access.reimbursementSubmit
    ...
    <div className={styles.directorySection}>
      <div className={styles.sectionHeader}>
        <div>
          ...
          <h1 className={styles.sectionTitle}>Reimbursement Claim</h1>
        </div>
                ...
                <div className={styles.reimbursementCardStack}>
                  {column.key === 'APPROVED'
                    ...
                    : column.items.map((item) => (
                        <article key={item.id} className={styles.reimbursementCard}>
                          <div className={styles.reimbursementCardTop}>
                            <div>
                          ...
                          <p className={styles.reimbursementCardMeta}>{formatDate(item.expense_date)}</p>
      ...
      {selectedClaim ? (
        <div className={styles.modalOverlay} onClick={closeClaimModal}>
          <div className={`${styles.modalCard} ${styles.reimbursementClaimModal}`.trim()} onClick={(event) => event.stopPropagation()}>
            ...
            <div className={styles.buttonRow}>
              {selectedClaim.status === 'SUBMITTED' && isClaimOwner(selectedClaim) ? (
                <button
                  type="button"
